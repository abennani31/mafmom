...
export class ContratComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.contratService
      ...
      .subscribe(
        (res: HttpResponse<IContrat[]>) => this.paginateContrats(res.body, res.headers),
  ...
  registerChangeInContrats() {
    this.eventSubscriber = this.eventManager.subscribe('contratListModification', response => this.reset());
  }
